...
def random_data_selection(
    dataset, num_compounds, num_confirmations, random_seed
):
    """
    ...
    for idx in idxs:
        ...
        if len(compound["formation_energy"]) >= num_confirmations:
            new_energy = []
            new_pos = []
            energy_idxs = np.random.randint(
                0, len(compound["formation_energy"]), num_confirmations
            )
            ...
            for i, e in enumerate(d["formation_energy"]):
                ...
                if type(model) == SchNet:
                    ...
                    for n in range(torch.cuda.device_count()):
                        ...
                        targets.append(
                            torch.tensor(e, device=device).reshape((1,))
                        )
...
def init_data(params):
    ...
    spl = random_data_selection(
        spl_full,
        params["num_compounds"],
